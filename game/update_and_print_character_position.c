...
/*   Updated: 2024/09/30 20:06:48 by tchobert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_character_on_exit(t_game_data *game_data, unsigned int *old_character_coords)
{
	if (game_data->character_data.is_right == true)
	{
		draw_image(game_data, &game_data->images_data.character_on_exit_right_img,
					game_data->character_data.character_coord[0],
					game_data->character_data.character_coord[1]);
	}
	else
	{
		draw_image(game_data, &game_data->images_data.character_on_exit_left_img,
					game_data->character_data.character_coord[0],
					game_data->character_data.character_coord[1]);
	}
...
{
    // Vérifier si le personnage est sur la sortie
    ...
    {
        // Si l'ancienne position était sur la sortie, dessiner l'image de sortie
        if (game_data->map_data.map_array[old_character_coords[0]][old_character_coords[1]] == EXIT_ITEM)
        {
            // Dessiner l'image de sortie
            draw_image(game_data, &game_data->images_data.exit_img, old_character_coords[0], old_character_coords[1]);
        }
        else
        {
            // Dessiner l'image du sol si on n'est pas sur la sortie
            draw_image(game_data, &game_data->images_data.floor_img,
                       old_character_coords[0], old_character_coords[1]);
        }
...
// 		print_character_on_exit(game_data, old_character_coords);
// 	}
// 	else if (game_data->map_data.map_array[old_character_coords[0]][old_character_coords[1]] == EXIT_ITEM)
// 	{
// 		draw_image(game_data, &game_data->images_data.exit_img, old_character_coords[0], old_character_coords[1]);
// 	}
// 	else
