...
/*   Updated: 2024/09/19 19:55:29 by tchobert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (is_full_wall(last_line) == false
		|| is_correct_size(last_line, map_data) == false)
		return (INVALID_LINE);
	return (VALID_LINE);
}
...
{
	if ((is_correct_size(line, map_data) == false)
		|| (is_surrounded_by_walls(line, map_data) == false))
	{
		return (INVALID_LINE);
	}
