...
/*   Updated: 2024/10/03 19:23:33 by tchobert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	load_items_images(t_game_data *game_data)
{
	if (load_image(game_data, &game_data->images_data.collectible_img,
			"./sources/assets/exp_book.xpm") == EXIT_FAILURE)
		return (EXIT_FAILURE);
	if (load_image(game_data, &game_data->images_data.exit_img,
			"./sources/assets/exit_stairs.xpm") == EXIT_FAILURE)
		return (EXIT_FAILURE);
	if (load_image(game_data, &game_data->images_data.wall_img,
			"./sources/assets/bookshelf_resized.xpm") == EXIT_FAILURE)
		return (EXIT_FAILURE);
	if (load_image(game_data, &game_data->images_data.floor_img,
			"./sources/assets/test_wood2.xpm") == EXIT_FAILURE)
		return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
